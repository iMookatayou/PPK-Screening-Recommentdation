...
interface Question6Props {
  ...
  onResult: (result: Question6Result) => void
}

export default function Question6_UTI({ frontAgentGender = '1', onResult }: Question6Props) {
  ...
  useEffect(() => {
    ...
    if (gender === '1') {
      ...
      if (hasUrinaryRetention) {
        ...
        noteParts.push('ปัสสาวะไม่สุด/ปัสสาวะขัด')
      }
    } else if (gender === '2') {
      clinic = ['med']
    } else {
  ...
  }, [gender, hasProstateIssue, hasUrinaryRetention, customClinic, extraNote])
    ...
    <div className="space-y-4 text-sm">
      ...
      <div>
        <label htmlFor="genderSelect" className="block font-medium text-gray-700 mb-1">
          เพศของผู้ป่วย (ดึงจากบัตรหรือเลือก):
        </label>
        <select
          id="genderSelect"
          value={gender}
        ...
        >
          ...
          <option value="3">เพศของผู้ป่วย ไม่ระบุ</option>
        </select>
      </div>
      ...
      <div>
        ...
        <textarea
          ...
          placeholder="ระบุหมายเหตุเพิ่มเติม เช่น ปวดหน่วงมาก"
        />
      </div>
    </div>
  )
