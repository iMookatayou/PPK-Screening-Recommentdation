// src/app/print-form/preview.tsx
'use client'
      ...
      <table className="w-full border border-gray-300 mb-6">
        ...
        <tbody>
          {Object.entries(answers).map(([key, data]) => {
            ...
            const clinic = data.clinic || 'ไม่ระบุ'

            return (
              <tr key={key} className="border-t">
                <td className="border px-3 py-2">{qId}</td>
                <td className="border px-3 py-2">{data.title || 'ไม่ระบุ'}</td>
                <td className="border px-3 py-2">{data.note || '—'}</td>
